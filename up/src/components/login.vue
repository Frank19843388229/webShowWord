<template>
  ...
  <div class="form-area">
    ...
    @keyframes move {
      ...
      <div class="form-item">
        <label for="username" v-focus class="form-label">用户名</label>
        <input
          type="text"
          id="username"
    ...
    <span class="register" @click="router.push({name:'register'})">没有账号？去注册</span>
  </div>
  </div>
</template>
...
function login() { 
  if (user.username === 'admin' && user.password === '123456') {
    // 传递实际的值给setData
    user.setData(user.username, user.password);
    router.push({ name: 'index' });
  } else {
    errorStr.value = '用户名或密码错误';
  }
}
const vFocus={
  mounted(el) { 
